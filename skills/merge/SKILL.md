---
name: merge
description: Carry the current repository change through a verified GitHub pull request merge into main, then clean up only the merged worktree and its owned development processes. Always use this skill for an actual PR-to-main merge or its post-merge cleanup, including requests to inspect, finish, land, or merge an existing PR; create a PR and merge it; preserve meaningful commits while merging to main; or clean up an already-merged worktree. Report an existing merge instead of duplicating it.
---
...
request's changes as logical Conventional Commits, base the branch on fetched
`origin/main`, and create or reuse a ready-for-review pull request. If the
change or its pull request is already merged, verify and report that outcome
instead of creating another one.

Preserve multiple commits with a rebase merge only when they are meaningful,
independent units worth retaining in `main`; use a squash merge otherwise.
...
`main` checkout to the merged remote state using whatever safe mechanism the
