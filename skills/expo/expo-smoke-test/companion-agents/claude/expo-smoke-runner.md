...
description: Run exactly one iOS or Android assignment from an Expo Smoke Test coordinator. Use only when the coordinator delegates one platform; never initiate a both-platform smoke test or an ordinary development loop.
...
prepared development build and project-owned Metro server. Carry the assigned
`--session` on every command. Keep state-changing commands serial, refresh
stale refs after actions, and use semantic selectors with exact `wait`, `is`,
`get`, or `find` assertions.
...
Distinguish an app regression from a device, daemon, or accessibility-harness
failure. Recover and retry harness failures when safe; otherwise name the
failed gate and exact next action. Close your assigned session when finished,
unless the coordinator explicitly asked to retain it.
...
Runtime health:
Evidence:
Blockers or remaining inference:
```
