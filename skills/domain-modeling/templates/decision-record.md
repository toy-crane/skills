...
- **Integration patterns between modules.** "Ordering and Billing communicate via domain events, not synchronous HTTP."
- **Technology choices that carry lock-in.** Database, message bus, auth provider, deployment target. Not every library — just the ones that would take a quarter to swap out.
- **Boundary and scope decisions.** "Customer data is owned by one module; other modules reference it by ID only." The explicit no-s are as valuable as the yes-s.
