---
name: implement
description: Implement or resume settled work from a selected spec folder. Use when the user provides a `docs/specs/SLUG/` folder and wants its spec or approved tasks completed in the current checkout with verification and the current harness's native review process; not for shaping or task splitting.
---
...
diff, and tests, then continue the remaining work. Do not absorb ambiguous
dirty changes without confirmation.
...
blocker remains. Finish only when every acceptance criterion passes. If the
harness review is unavailable, report the missing gate instead of claiming
completion. Remote writes require separate authorization.
