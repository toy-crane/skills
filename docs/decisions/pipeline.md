...
- The issue is a pointer, never a second copy of the contract. Its body is
  ...
  every merge that changes the folder, so it carries enough to recognize the
  spec in the tracker without becoming a source that drifts. The folder stays
  the only contract; the tracker holds only the claim, the blocking edges, and
  open or closed state. `implement` takes the folder as input and finds the
  issue by the `spec:<slug>` title key, matched exactly. Cross-folder ordering
  is a `Blocked by: docs/specs/<other>/` line in `spec.md`, written whether
  or not a tracker exists, and `implement` leaves a `Spec-Folder` commit
  trailer so `pr` and `merge` can name the issue to close.
...
- The standard task workflow inside one spec folder is sequential. Parallel
  ...
  write, and the implementation pull request closes it. Without the section
  every skill behaves as before. Parallel bulk migrations or independent task
  queues inside one folder still require a separately chosen execution model.
...
- Parallel spec work needs task-level claims inside one folder, or two
  sessions on one tracker account collide often enough that the assignee
  field no longer separates them.
