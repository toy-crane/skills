...
- `setup-issue-tracker` keeps the always-loaded tracker section focused on
  purpose, explicit user authority to resume active work, and PR-linked closure.
  It routes issue work to `docs/issue-tracker.md`, which holds repository-specific
  keys, tools, claim states, closing behavior, and operations. Exact argument
  lists come from current tool help; general guidance stays with the skill.
...
- The issue is a pointer, never a second copy of the contract. Its body is
  derived from `spec.md` by document structure and regenerated by `merge` on
  every merge that changes the folder, while `maintain-project-context`
  periodically lists every managed issue and repairs the full mirror against
  the fetched remote default branch. This keeps enough context to recognize the
  spec without making the tracker a source that can drift. The folder stays the
  only contract; the tracker holds only the claim, the blocking edges, and open
  or closed state. `implement` takes the folder as input and finds the issue by
  the `spec:<slug>` title key, matched exactly. Cross-folder ordering is a
...
- The standard task workflow inside one spec folder is sequential. Parallel
  ...
  `merge` publishes one pointer issue per `docs/specs/<slug>/` folder as it
  lands on the default branch, `implement` claims that issue as its first
  write, and the implementation pull request closes it.
...
- Parallel spec work needs task-level claims inside one folder, or the active
  state and explicit resumption rule cannot prevent competing claims.
- Merges made outside `merge` leave folders unpublished long enough that
  sessions claim the same folder before the next catch-up.
...
- Copying the spec into the issue, as Matt Pocock's `to-spec` does — the
  folder holds several documents an issue cannot carry, and a hand-written
  copy drifted in this repository, where three of nine queued specs were
  edited after they were written.
- Publishing the issue when shaping closes — the folder is not yet on the
  default branch, so the issue would point at nothing until the spec merges.
...
- A claim marker inside the folder — a file written on a branch is invisible
