...
behind shape-idea (where shape-idea-names-the-work rejected `write-spec` because writing was the
byproduct), plan.md is itself the review surface, so writing it is an honest
name for producing it.

Rename `draft-plan` to `write-plan`. The verb-object scheme (verb-object-names-for-invoked-skills) is
...
- **to-plan** (rejected, again): a preposition with the verb elided; a route
  ...
  the to-X lineage. Also verb-object-names-for-invoked-skills's original starting point.
...
- **Bare plan** (impossible): `/plan` is reserved by both harnesses (verb-object-names-for-invoked-skills).
...
reinstalled, as with the verb-object-names-for-invoked-skills and shape-idea-names-the-work renames.
