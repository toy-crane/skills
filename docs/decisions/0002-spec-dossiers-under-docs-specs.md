...
(`plan.md`, `tasks.md`, …) join under fixed filenames — one address hands off the whole
dossier. The parent is named after the anchor document because everything in the folder
elaborates one spec (plan derives from spec, tasks from plan), and it exists because
specs are a third document lifecycle: `docs/decisions/` is append-only and immutable,
`GLOSSARY.md` is permanent and current, while a spec dossier lives per unit and retires
wholesale when the work ships. Mixing lifecycles in one folder would break the reader's
trust in what each path promises.
